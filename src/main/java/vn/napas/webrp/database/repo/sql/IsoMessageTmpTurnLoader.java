			...
			String sql = """
					...
					SELECT
					  ...
					  CASE UPPER(TRIM(CURRENCY_CODE))
					    ...
					    WHEN 'VDD' THEN 704
					    ELSE CASE
					           WHEN TRIM(CURRENCY_CODE) REGEXP '^[0-9]+$' THEN CAST(TRIM(CURRENCY_CODE) AS UNSIGNED)
					           ELSE NULL
					         END
					  ...
					  CASE
					    ...
					    ELSE TRIM(IBFT_INFO)
					  END AS IBFT_INFO,
					  ACQ_INST_COUNTRY_CODE AS ACQ_COUNTRY,
					  POS_ENTRY_MODE        AS POS_ENTRY_CODE,
					  POS_CONDITION_CODE,
					  TRANX_REF             AS ADDRESPONSE,
					  SERVICE_CODE          AS MVV,
					  AMOUNT                AS F4,
					  AMOUNT_SETTLEMENT     AS F5,
					  AMOUNT_CARDHOLDER     AS F6,
					  /* F49 (INT) */
					  CASE UPPER(TRIM(CURRENCY_CODE))
					    WHEN 'USD' THEN 840
					    WHEN 'VND' THEN 704
					    ...
					    ELSE CASE
					         ...
					         END
					  END AS F49,
					  /* SETTLEMENT_CODE (INT) – mô phỏng DECODE '   ' và '.  ' */
					  CASE CURRENCY_CODE_SETTLEMENT
					    WHEN '   ' THEN 704
					    WHEN '.  ' THEN 704
					    ELSE CASE
					           WHEN TRIM(CURRENCY_CODE_SETTLEMENT) REGEXP '^[0-9]+$'
					             THEN CAST(TRIM(CURRENCY_CODE_SETTLEMENT) AS UNSIGNED)
					           ELSE NULL
					         END
					  END AS SETTLEMENT_CODE,
					  CONVERSION_RATE_SETTLEMENT  AS SETTLEMENT_RATE,
					  CONVERSION_RATE_CARDHOLDER  AS ISS_CONV_RATE,
					  USER_DEFINE                 AS TCC,
