 ...
 *
 * Ghi chú QUAN TRỌNG cho TiDB/MySQL:
 * 1) TRUNCATE là DDL => auto-commit. Nếu muốn giữ toàn bộ thao tác trong 1 transaction, dùng DELETE FROM ... thay cho TRUNCATE.
 * 2) Định dạng ngày: dùng STR_TO_DATE(:str, '%d/%m/%Y') và DATE_FORMAT(dt,'%d/%m/%Y') khi cần.
 * 3) Cố gắng gom các bước thành những câu INSERT ... SELECT lớn, giữ nguyên semantics từ Oracle.
 * 4) Logging thay cho INSERT vào ERR_EX có thể thực hiện trực tiếp bằng JDBC; nếu cần, map sang entity JPA sau.
 */
...
public class Proc_NAPAS_MASTER_VIEW_DOMESTIC_IBFT {
    ...
    public int insertSessionDomesticIbft(LocalDate from, LocalDate to, String user) {
        // Nếu đã có bản TiDB tương đương procedure INSERT_TCKT_SESSION_DOMESTIC_IBFT thì thay câu dưới bằng CALL ...
        // Ở đây minh hoạ: ghi dấu vào bảng session (hoặc view materialized tạm) – tuỳ implement thực tế của bạn.
        String sql = """
        ...
        """;
        MapSqlParameterSource p = new MapSqlParameterSource()
                .addValue("from", from.format(DMY))
                .addValue("to", to.format(DMY))
                .addValue("user", user);
    ...
    public int insertBlock_SuccessByRole(LocalDate from, LocalDate to) {
        // Đây là KHỐI 1: ví dụ giao dịch thành công (RESPCODE=0), gom theo vai trò ISS/ACQ
        // Lưu ý: Đây là skeleton. Bạn hãy thay SELECT chi tiết theo store gốc.
        String sql = """
            ...
            SELECT
                -- Ví dụ map BANK_ID đặc thù (hãy mở rộng theo rule thực tế)
                CASE
                ...
                COALESCE(LIQUIDITY,'') AS LIQUIDITY,
                /* clamp settlement_date vào khoảng truy vấn */
                CASE
            ...
            GROUP BY
                ...
                COALESCE(SERVICE_CODE,''), COALESCE(LIQUIDITY,''),
                CASE
                    WHEN SETTLEMENT_DATE < STR_TO_DATE(:from, '%d/%m/%Y') THEN STR_TO_DATE(:from, '%d/%m/%Y')
                    WHEN SETTLEMENT_DATE > STR_TO_DATE(:to, '%d/%m/%Y') THEN STR_TO_DATE(:to, '%d/%m/%Y')
                    ELSE SETTLEMENT_DATE
    ...
    public int insertBlock_ErrorAdjust(LocalDate from, LocalDate to) {
        // KHỐI 2: ví dụ xử lý giao dịch lỗi/điều chỉnh (RESPCODE <> 0), sử dụng EDIT_DATE nếu cần
        String sql = """
            INSERT INTO TCKT_NAPAS_IBFT(
                BANK_ID, GROUP_ROLE, GROUP_TRAN, TRAN_TYPE,
                DB_TOTAL_TRAN, DB_AMOUNT, DB_IR_FEE, DB_SV_FEE, DB_TOTAL_FEE, DB_TOTAL_MONEY,
            ...
            SELECT
                ...
                COALESCE(SERVICE_CODE,'') AS SERVICE_CODE,
                COALESCE(LIQUIDITY,'') AS LIQUIDITY,
                /* Dùng EDIT_DATE nếu có, nếu không dùng SETTLEMENT_DATE; sau đó clamp về khoảng truy vấn */
                CASE
