...
public class Proc_NAPAS_SHC_TMP_DOMESTIC_IBFT {
	...
	@Autowired Proc_DATA_TO_ECOM proc_DATA_TO_ECOM;
	@Autowired Proc_GET_BCCARD_ID proc_gaBccard_ID;
...
//				    END AS AMOUNT,
//				    t.SETTLEMENT_AMOUNT,
//				    CASE WHEN t.acquirer_rp = :bankcardid THEN t.CARDHOLDER_AMOUNT ELSE t.TRANSACTION_AMOUNT END AS PRE_CARDHOLDER_AMOUNT,
...
//		jdbc.update(sql, ps);
	...
	private void recomputeFeesStage1_v() {
		// Block UPDATE #1: set Fee_Key, FEE_ISS, FEE_ACQ, FEE_IRF_ISS... theo công thức
		// NAPAS_* (giữ nguyên hàm DB)
		// LƯU Ý: Đã đổi DECODE -> CASE, NVL -> IFNULL, TRUNC/TO_CHAR -> MySQL
		// equivalents.
	...
	private void recomputeFeesStage3_ByWhitelist() {
		...
		String sql = """
				...
				""";
		log.info("recomputeFeesStage3_ByWhitelist: {}", sql);
		jdbc.update(sql, new MapSqlParameterSource());
	}
