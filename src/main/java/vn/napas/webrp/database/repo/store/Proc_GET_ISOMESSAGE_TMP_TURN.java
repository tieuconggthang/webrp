...
//    private final DbLoggerRepository dblog;
	...
	private long getWatermarkStt() {
		final String sql = "SELECT GREATEST(" + "  IFNULL((SELECT MAX(STT) FROM SHCLOG), 0),"
...
//    }
	...
	/**
	 * Thực thi nghiệp vụ GET_ISOMESSAGE_TMP_TURN: đổ 0210 từ ISOMESSAGE_TMP_TURN
	 * sang SHCLOG_SETT_IBFT kèm sinh STT = iSTT + rownum và chuyển đổi cột theo các
	 * quy tắc trong store gốc.
	 */
	@Transactional
	public int execute() {
...
//        ensureIndexes();
		...
		// * NP_CONVERT_LOCAL_DATE(x, TRUNC(SYSDATE)) -> convertLocalDate(x) :
		// STR_TO_DATE cho định dạng số; NULL -> CURDATE()
		// * MAP_IBFT_ACQ_ID(ACQ_ID) -> m.map_id từ bảng map (nếu có), fallback về
		// ACQ_ID
		// * GET_IBT_BIN(BEN_ID hoặc SUBSTR(DEST_ACCOUNT,1,6)) -> tra bảng BIN (nếu có),
		// fallback NULL
		// * to_number_bnv(BEN_ID) -> CAST(BEN_ID AS DECIMAL) an toàn; nếu không numeric
		// -> NULL
		//
		// - Kiểm tra số: IS_NUMBER(col) -> col REGEXP '^[0-9]+$'
		// - TRUNC(date) -> DATE(date) ; TO_CHAR(ts,'HH24MISS') ->
		// DATE_FORMAT(ts,'%H%i%s')
		//
...
//            "  AND (TRACE_NO REGEXP '^[0-9]+$')";
		...
		final String sql = """
				...
				SELECT
				  ...
				  VAS_INFO,                        -- TXNSRC (giữ đúng store gốc)
				...
				FROM (
				  ...
				  FROM (
				    SELECT
				      ...
				      /* RESPCODE seed */
					CASE
					  WHEN t.BEN_ID = '971133'
					       AND (t.DEST_ACCOUNT LIKE 'NPDC%' OR t.DEST_ACCOUNT LIKE 'NQ%')
					    THEN CASE
					           WHEN t.RESPONSE_CODE REGEXP '^[0-9]+$'
					             THEN CAST(t.RESPONSE_CODE AS UNSIGNED)
					           ELSE 68
					         END
	
					  WHEN t.SERVICE_CODE = 'QR_PUSH'
					    THEN CASE
					           WHEN t.RESPONSE_CODE REGEXP '^[0-9]+$'
					             THEN CAST(t.RESPONSE_CODE AS UNSIGNED)
					           ELSE 68
					  ...
					  WHEN t.BEN_ID = '971100' AND t.TCC = '99'
					    THEN CASE
					           WHEN t.RESPONSE_CODE REGEXP '^[0-9]+$'
					             THEN CAST(t.RESPONSE_CODE AS UNSIGNED)
					           ELSE 68
					  ...
					  WHEN TRIM(t.ISS_ID) IN ('980471','980472')
					    THEN CASE
					           WHEN t.RESPONSE_CODE REGEXP '^[0-9]+$'
					             THEN CAST(t.RESPONSE_CODE AS UNSIGNED)
					           ELSE 68
					  ...
					  WHEN t.RESPONSE_CODE = '68'
					    THEN 0   -- chuyển thẳng về 0
					...
					END AS RESPCODE_SEED,
				      ...
				      CASE WHEN CAST(t.ACQ_ID AS UNSIGNED)=191919 THEN 970459
				           ...
				           WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970429 THEN 157979
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970400 THEN 161087
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970427 THEN 166888
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970441 THEN 180906
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970425 THEN 970459
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970431 THEN 452999
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970436 THEN 686868
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970419 THEN 818188
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970407 THEN 888899
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970434 THEN 888999
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970440 THEN 970468
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970418 THEN 970488
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970432 THEN 981957
						   WHEN CAST(t.ACQ_ID AS UNSIGNED) = 970405 THEN 970499
...
//        	SqlLogUtils.renderSql(sql, Map.of("STT", iSTT));
			log.info(SqlLogUtils.renderSql(sql, Map.of("STT", iSTT)));
