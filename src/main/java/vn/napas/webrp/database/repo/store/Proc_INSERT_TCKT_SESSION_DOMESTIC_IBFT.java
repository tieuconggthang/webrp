...
public class Proc_INSERT_TCKT_SESSION_DOMESTIC_IBFT {
	...
	private static final String SQL_BACKUP_SESSION = """
			    INSERT INTO TCKT_SESSION_DOMESTIC_AUTO_BKUP(
			      ...
			      CREDIT, DB_AMOUNT, DB_IR_FEE, DB_SV_FEE, DB_TOTAL_FEE, DIRECTION, FEE_TYPE,
			      GROUP_ROLE, GROUP_TRAN, IS_FEE, LIQUIDITY, MERCHANT_TYPE, MSGTYPE_DETAIL,
			      NAPAS_FEE, NP_ADJ_FEE, OD_BY, PART_FE, PCODE, PCODE2, SERVICE_TYPE, SETTLEMENT_CURRENCY,
			      SETT_DATE, STEP, SUB_BANK, TRAN_TYPE, WITH_BANK
			    )
			    ...
			    WHERE SETT_DATE BETWEEN STR_TO_DATE(:fromDate, '%d/%m/%Y')
	...
	private static final String SQL_DELETE_OLD_SESSION = """
			    ...
			    WHERE (DATE(INSERT_DATE) <= DATE(:lastSettDate) OR INSERT_DATE > NOW())
	...
	private static final String SQL_STEP_1_ISS_ACQ = """
			...
			SELECT
			  ...
			  CASE
			    ...
			    WHEN Respcode = 0 AND SETTLEMENT_DATE BETWEEN STR_TO_DATE(:fromDate,'%d/%m/%Y') AND STR_TO_DATE(:toDate,'%d/%m/%Y') THEN SETTLEMENT_DATE
			    ELSE NULL
			  END AS SETT_DATE,
			  CASE
			    WHEN Respcode = 0 THEN NULL
			    ELSE CASE WHEN DATE(Edit_Date) < STR_TO_DATE(:fromDate,'%d/%m/%Y') THEN STR_TO_DATE(:fromDate,'%d/%m/%Y') ELSE DATE(Edit_Date) END
			  END AS EDIT_DATE,
			  CASE WHEN ACQ_CURRENCY_CODE = 840 THEN 840 WHEN ACQ_CURRENCY_CODE = 418 THEN 418 ELSE 704 END AS SETTLEMENT_CURRENCY,
			  RESPCODE,
			  CASE
			...
			GROUP BY
			  ...
			  CASE
			    WHEN ISSUER_RP IN (220699, 602907, 605609, GET_BCCARD_ID()) THEN ISSUER_RP -- note: this expression differs by role; retained per grouping need
			    WHEN PCode2 IN (810000,820000,830000,860000,870000) THEN 999999
			    ELSE 0
			  END,
			  CASE
	...
	private static final String SQL_STEP_2_ACQ_ISS = """
			...
			SELECT
			  ...
			  END AS GROUP_TRAN,
			  CASE
			    WHEN PCODE2 IN (960000,970000,980000,990000,967500,977500,967600,977600,968400,978400,\
			968500,978500,987500,997500,987600,997600,988400,998400,988500,998500,\
			                    967700,977700,987700,997700,967800,977800,987800,997800,967900,977900,987900,997900,\
			                    966100,976100,986100,996100,966200,976200,986200,996200)
			      THEN CONCAT(LPAD(CAST(PCODE AS CHAR),2,'0'), PCODE2)
			    ELSE LPAD(CAST(PCODE AS CHAR),2,'0')
	...
	private static final String SQL_STEP_3_ISS_BEN = """
			...
			-- ISS-BEN (TiDB)
			INSERT INTO TCKT_SESSION_DOMESTIC(
			  INSERT_DATE, SETT_DATE, EDIT_DATE, SETTLEMENT_CURRENCY, RESPCODE,
			  GROUP_TRAN, PCODE, TRAN_TYPE, SERVICE_CODE, GROUP_ROLE, BANK_ID, CORR_BANK_ID, WITH_BANK,
			  DB_TOTAL_TRAN, DB_AMOUNT, DB_IR_FEE, DB_SV_FEE, DB_TOTAL_FEE, DB_TOTAL_MONEY,
			  CD_TOTAL_TRAN, CD_AMOUNT, CD_IR_FEE, CD_SV_FEE, CD_TOTAL_MONEY,
			...
			SELECT
			  ...
			  SUM(
			    CASE
			      WHEN Respcode IN (112,114) THEN IFNULL(PREAMOUNT,0)
			      ELSE CASE WHEN ACQ_CURRENCY_CODE IN (418,764) THEN SETTLEMENT_AMOUNT ELSE AMOUNT END
			    END
			  ) AS DB_AMOUNT,
			  SUM(
			    CASE
	...
	private static final String SQL_STEP_5_BEN_ISS = """
			...
			SELECT
			  ...
			  SUM(CASE WHEN Respcode IN (112,114) THEN IFNULL(PREAMOUNT,0) ELSE AMOUNT END) AS CD_AMOUNT,
			  SUM(CASE
	...
	private static final String SQL_STEP_6_ADJ_ISS_ACQ = """
			...
			INSERT INTO TCKT_SESSION_DOMESTIC (
			  ...
			  GROUP_TRAN, PCODE, TRAN_TYPE, SERVICE_CODE, GROUP_ROLE, BANK_ID, CORR_BANK_ID, WITH_BANK,
			  DB_TOTAL_TRAN, DB_AMOUNT, DB_IR_FEE, DB_SV_FEE, DB_TOTAL_FEE, DB_TOTAL_MONEY,
			  CD_TOTAL_TRAN, CD_AMOUNT, CD_IR_FEE, CD_SV_FEE, CD_TOTAL_MONEY, NAPAS_FEE, ADJ_FEE, NP_ADJ_FEE,
			  MERCHANT_TYPE, BC_NP_SUM, BC_CL_ADJ, STEP, FEE_TYPE, PART_FE, SERVICE_TYPE
			)
			SELECT
			  NOW() AS INSERT_DATE,
			  MSGTYPE_DETAIL,
			...
			GROUP BY
			  ...
			  CASE
			    WHEN Respcode = 0 THEN NULL
			    ELSE CASE WHEN DATE(Edit_Date) < Sett_From THEN Sett_From ELSE DATE(Edit_Date) END
			  END,
			  CASE WHEN ACQ_CURRENCY_CODE = 840 THEN 840 WHEN ACQ_CURRENCY_CODE = 418 THEN 418 ELSE 704 END,
			  RESPCODE,
			  CASE
	...
	private static final String SQL_STEP_8_ADJ_ISS_BEN = """
			...
			INSERT INTO TCKT_SESSION_DOMESTIC (
			    INSERT_DATE, SETT_DATE, EDIT_DATE, SETTLEMENT_CURRENCY, RESPCODE,
			    GROUP_TRAN, PCODE, TRAN_TYPE, SERVICE_CODE, GROUP_ROLE,
			    BANK_ID, CORR_BANK_ID, WITH_BANK,
			    DB_TOTAL_TRAN, DB_AMOUNT, DB_IR_FEE, DB_SV_FEE, DB_TOTAL_FEE, DB_TOTAL_MONEY,
			    CD_TOTAL_TRAN, CD_AMOUNT, CD_IR_FEE, CD_SV_FEE, CD_TOTAL_MONEY,
			    NAPAS_FEE, ADJ_FEE, NP_ADJ_FEE, MERCHANT_TYPE, STEP, FEE_TYPE, SERVICE_TYPE
			)
			SELECT
			    ...
			    /* SETT_DATE */
			    CASE
			        WHEN RESPCODE = 0 AND SETTLEMENT_DATE < ? THEN ?
			        WHEN RESPCODE = 0 AND SETTLEMENT_DATE > ? THEN ?
			        WHEN RESPCODE = 0 AND SETTLEMENT_DATE BETWEEN ? AND ? THEN SETTLEMENT_DATE
			        ELSE NULL
			    END AS SETT_DATE,
			    ...
			    /* FEE_TYPE (mapping theo ISSUER_FE/PCODE2) */
			    CASE
			        WHEN PCODE2 = 930000 AND ISSUER_FE = 130001 THEN 'COVID_FEE'
			        WHEN PCODE2 = 930000 AND ISSUER_FE = 130012 THEN 'QR_IBFT_FEE'
			        WHEN PCODE2 = 950000 AND ISSUER_FE = 130001 THEN 'COVID_FEE'
			...
			/* GROUP BY tất cả non-aggregates */
			GROUP BY
			    CASE
			        WHEN RESPCODE = 0 AND SETTLEMENT_DATE < ? THEN ?
			        WHEN RESPCODE = 0 AND SETTLEMENT_DATE > ? THEN ?
			    ...
			    CASE
			        WHEN RESPCODE = 0 THEN NULL
			        ELSE CASE WHEN DATE(Edit_Date) < ? THEN ? ELSE DATE(Edit_Date) END
			    END,
			    CASE
			        WHEN ACQ_CURRENCY_CODE = 840 THEN 840
			        WHEN ACQ_CURRENCY_CODE = 418 THEN 418
	...
	private static final String SQL_STEP_9_ADJ_BEN_ISS = """
			...
			SELECT
			    ...
			    /* BEN-ISS: DB* phần lớn = 0, chỉ có CD* là tổng hợp */
			    0 AS DB_TOTAL_TRAN,
			    0 AS DB_AMOUNT,
	...
	private static final String SQL_STEP_10 = """
			...
			WHERE
			  ...
			  AND BANK_ID  NOT IN (600005, GET_BCCARD_ID())
			  AND WITH_BANK NOT IN (600005, GET_BCCARD_ID())
	...
	private static final String SQL_STEP_16 = """
			...
			WHERE
			  SERVICE_TYPE = 'IBFT'
			  AND INSERT_DATE > CURDATE();    -- nếu INSERT_DATE là DATETIME
			--AND INSERT_DATE >= CURDATE();   -- nếu INSERT_DATE là DATE
...
//			""";
	...
	public void run(String fromDate_ddMMyyyy, String toDate_ddMMyyyy, String user) {
		MapSqlParameterSource p = new MapSqlParameterSource().addValue("fromDate", fromDate_ddMMyyyy)
				.addValue("toDate", toDate_ddMMyyyy).addValue("user", user);
	...
	private void exec(String tag, String sql, MapSqlParameterSource p) {
		...
		if (trimmed.isEmpty() || trimmed.startsWith("/* TODO")) {
